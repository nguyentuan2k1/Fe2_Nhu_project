...
var end = false;
var rdCreateBall ;
// code logic refesh and level up ... 1
var time_Ball_Fall = 12;
...
var interval = setInterval(()=>{
 ...
 if(level<4){
    ...
    if(rdCreateBall<0.8 && !document.querySelector('#pause').checked ){
        createbuble('rgb(102, 255, 0)',time_Ball_Fall,'none');
    }
    else if(rdCreateBall>=0.8 && rdCreateBall<0.9 && !document.querySelector('#pause').checked){
        createbuble('rgb(255, 0, 64)',time_Ball_Fall,'yes');
    }
...
var header = $('cover-all');
var backgrounds = new Array(
    'https://img4.thuthuatphanmem.vn/uploads/2020/12/26/anh-nen-phong-canh-nhat-ban-dep_125911230.jpg'
  , 'https://img4.thuthuatphanmem.vn/uploads/2020/12/26/hinh-nen-nhat-ban-full-hd_125914983.jpg'
  , 'https://img4.thuthuatphanmem.vn/uploads/2020/12/26/hinh-nen-nhat-ban-hoa-anh-dao_125915181.jpg'
  ,'https://img4.thuthuatphanmem.vn/uploads/2020/12/26/hinh-nen-nhat-ban-mua-dong_125915388.jpg'
);
function nextBackground() {
    let backgroundRand = backgrounds[Math.floor(Math.random() * backgrounds.length)];
    document.querySelector(`.cover-all`).style.backgroundImage = `url(${backgroundRand})`;
}
//Đổi màu bóng
var mamau="";
...
function changeColor(){
    const bubble = document.querySelectorAll('.buble'); // cái này nó sẽ là biến mảng  nên phải duyệt 
    const arrayColor = ['pink','blue','gray','green','yellow'];
    let rd = arrayColor[randomColor(arrayColor)];
    mamau = rd ;
    bubble.forEach(function(tung_qua_bong){
        tung_qua_bong.style.backgroundColor = rd;
    })
